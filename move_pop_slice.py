...
if __name__ == '__main__':
    ...
    print(r'读取交通分区......')
    # 交通分区
    traffic_district_gdf = gpd.read_file(r'./data/input/全交通分区.shp')
    print(r'读取交通分区完成......')

    print(r'读取市级行政区划......')
    # 广东市级行政区划
    guangdong_gdf = gpd.read_file(r'./data/input/广东省.json')
    print(r'读取市级行政区划完成......')
    ...
    for file in movement_inner_file_list + movement_out_in_file_list + movement_in_out_file_list:
        ...
        for _label in [i for i in range(1, N + 1)]:
            ...
            # 字段匹配，筛选佛山内外，匹配佛山内行政区划
            result = filter_od_df(start_grid_field='start_grid_id',
                                  end_grid_field='end_grid_id',
                                  region_grid_id_field='grid_id',
                                  target_region_df=grid_gdf,
                                  od_df=slice_df)
            del slice_df
            # 空间匹配，匹配起点交通区划ID
            result = df_spatial_join(geofile=traffic_district_gdf,
                                     od_df=result,
                                     od_df_geometry='start_geometry',
                                     join_column_name='id',
                                     output_column_name='start_traffic_district_id')
            # 空间匹配，匹配终点交通区划ID 
            result = df_spatial_join(geofile=traffic_district_gdf,
                                     od_df=result,
                                     od_df_geometry='end_geometry',
                                     join_column_name='id',
                                     output_column_name='end_traffic_district_id')
            # 空间匹配，匹配起点行政区划name
            result = df_spatial_join(geofile=guangdong_gdf,
                                     od_df=result,
                                     od_df_geometry='start_geometry',
                                     join_column_name='name',
                                     output_column_name='start_city_name')
            # 空间匹配，匹配终点行政区划name
            result = df_spatial_join(geofile=guangdong_gdf,
                                     od_df=result,
                                     od_df_geometry='end_geometry',
                                     join_column_name='name',
                                     output_column_name='end_city_name')

            result = result[['date', 'start_half',
        ...
        with open(rf'./data/input/join/move_join_{i}', 'wb') as f:
